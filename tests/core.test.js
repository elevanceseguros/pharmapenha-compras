...
test('não aumenta quantidade para completar mínimos quando há alternativas',()=>{const p=optimize([item('a')],[offer('1','a','X',20000),offer('2','a','Y',30000)],[supplier('X',60000),supplier('Y',60000)],params);assert.equal(p.status,'infeasible');assert.equal(p.orders.length,0)});
test('libera pedido abaixo do mínimo quando o produto só existe em um fornecedor',()=>{const p=optimize([item('a')],[offer('1','a','X',1200)],[supplier('X',50000)],params);assert.equal(p.status,'feasible');assert.equal(p.orders[0].valid,true);assert.equal(p.orders[0].minimumException,true);assert.equal(p.orders[0].shortfall,48800);assert.deepEqual(p.orders[0].exceptionProducts,['a'])});
test('continua exigindo mínimo quando o produto tem mais de um fornecedor',()=>{const p=optimize([item('a')],[offer('1','a','X',1200),offer('2','a','Y',1500)],[supplier('X',50000),supplier('Y',50000)],params);assert.equal(p.status,'infeasible')});
...
test('mínimo exclui frete e impostos por padrão',()=>{const offers=[{...offer('1','a','X',60000),netCents:50000},{...offer('2','a','Y',60000),netCents:50000}],suppliers=[supplier('X',60000,20000),supplier('Y',60000,20000)];assert.equal(optimize([item('a')],offers,suppliers,params).status,'infeasible');assert.equal(optimize([item('a')],offers,suppliers.map(s=>({...s,minimumBasis:'gross'})),params).status,'feasible')});
