...
test('PDF contém apenas o fornecedor escolhido e ZIP separa pedidos',()=>{
 const order={valid:true,supplier:{name:'Fornecedor A',freightCents:0,freightKnown:true},net:10000,gross:10650,tax:650,total:10650,lines:[{itemId:'item-a',description:'Castanha da Índia',reference:'COT-123',packs:1,packQty:100,unit:'g',unitGross:10650,gross:10650}]};
 ...
 const str=new TextDecoder('latin1').decode(bytes);assert.ok(str.startsWith('%PDF-'));assert.ok(str.includes('Fornecedor A'));assert.ok(!str.includes('USO INTERNO'));assert.ok(!str.includes('Gamma'));assert.ok(!str.includes('qualidade'));assert.ok(str.includes('106,50'));
 ...
 const internal=new TextDecoder('latin1').decode(internalReportPDF([order],audit,'Teste',new Date('2026-09-03T12:00:00Z')));assert.ok(internal.includes('USO INTERNO'));assert.ok(internal.includes('Fornecedor A'));assert.ok(internal.includes('Gamma'));assert.ok(internal.includes('qualidade'));assert.ok(!internal.includes('Concorrente secreto'));
