...
export function internalReportPDF(orders,audit,round,date=new Date()){
 ...
 header();para('Conferência consolidada dos pedidos efetivamente realizados. Este documento deve acompanhar a análise das notas fiscais e não deve ser enviado aos fornecedores.');y+=4;
 const grandTotal=orders.reduce((t,o)=>t+o.total,0);para(`Total geral previsto: ${money(grandTotal)}`,true);para(`${orders.length} fornecedor${orders.length===1?'':'es'} com pedido.`);y+=5;
 for(const order of orders){if(y>225)page();doc.setFillColor(210,231,241);doc.rect(15,y-5,180,11,'F');para(order.supplier.name,true);para(`Pagamento: ${order.payment||'a combinar'} | Frete: ${order.freightKnown?money(order.freightCents):'a confirmar'}`);for(const line of order.lines){const qty=line.qty??line.packs*line.packQty;para(`${line.product||line.description} | ${line.packs} x ${line.packQty} ${line.unit} | total ${qty} ${line.unit} | ${money(line.gross)}`)}para(`Produtos: ${money(order.gross)} | Total do pedido: ${money(order.total)}`,true);y+=5}
 ...
 const unselected=(audit.items||[]).filter(i=>i.enabled!==false&&!chosen.has(i.id));if(unselected.length){if(y>240)page();para('PRODUTOS SEM COMPRA SELECIONADA',true);for(const item of unselected)para(`${item.name} | necessidade ${item.qty} ${item.unit}`);y+=5}
 para('JUSTIFICATIVAS DE OFERTAS DESCONSIDERADAS',true);
