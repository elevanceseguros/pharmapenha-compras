...
export function parseQuotation(text){
 ...
 for(const raw of text.split(/\n/)){
  ...
  const standard=line.match(/^(.*?)\s+(Brasil|China|Índia|India)\s+(\d{2}\/\d{2}\/\d{4})\s+([\d.,]+)\s+([\d.,]+)\s*(KG|G|ML|L|UN)\s+(.*)$/i);
  if(standard){
   const p=[...standard[7].matchAll(/R\$\s*([\d.]+,\d{2})/g)].map(m=>decimal(m[1]));
   if(p.length>=4){const packs=decimal(standard[4]);const q=quantity(decimal(standard[5]),standard[6]);rows.push({description:standard[1],...q,net:Math.round(p[1]*100),gross:Math.round((packs?p.at(-1)/packs:p[1])*100),quotedPacks:packs,available:packs>0,validity:standard[3],source:line});continue}
  }
  ...
  const manual=line.match(/^(.*?)\s+(\d+(?:[.,]\d+)?)\s*(kg|mg|g|ml|l|un)\s*[-–—:;]?\s*(?:R\$\s*)?([\d.]+(?:,\d{1,2})?)\s*$/i);
